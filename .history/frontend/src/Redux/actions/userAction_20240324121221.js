...
export const updateVideoProgressAction = (videoProgress) => async (dispatch) => {
    dispatch({ type: GET_APPLICANT_PROGRESS_REQUEST });
    try {
        const { data } = await axios.post(`/api/user/updateVideoProgress`, videoProgress);
        dispatch({ 
            type: GET_APPLICANT_PROGRESS_SUCCESS, 
            payload: data 
        });
    } catch (error) {
        dispatch({
            type: GET_APPLICANT_PROGRESS_FAIL,
            payload: error.response.data?.error
        });
    }
