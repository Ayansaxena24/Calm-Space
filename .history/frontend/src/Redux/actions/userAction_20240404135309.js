...
export const updateVideoProgressAction = (videoUrl, isCompleted:true, userId) => async (dispatch) => {
    dispatch({ type: UPDATE_VIDEO_PROGRESS_REQUEST });
    console.log(userId,"asasdassasdasdasasassa");
    try {
        const { data } = await axios.post(`/api/user/updateVideoProgress`, { videoUrl, userId });
        dispatch({ 
