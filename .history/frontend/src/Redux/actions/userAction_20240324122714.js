...
export const updateVideoProgress = (videoProgress) => async (dispatch) => {
    dispatch({ type: UPDATE_VIDEO_PROGRESS_REQUEST });
    try {
        await axios.post('/api/user/updateVideoProgress', { videoProgress });
        dispatch({ type: UPDATE_VIDEO_PROGRESS_SUCCESS });
    } catch (error) {
        dispatch({
