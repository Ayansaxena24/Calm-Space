...
const SearchInputEl = () => {
    ...
    return (
        ...
        <form onSubmit={handleSubmit} style={{ width: '80%'}} className='flex justify-center ml-72 min-h-[100vh]'>
            <Box 
            className='flex w-[70%] justify-center'
            sx={{textAlign:'right'}}
            >
                <InputBase className={`p-2 ${mode === 'dark' ? 'bg-gray-900' : 'bg-[#fbf7f0]'} }`} 
                // sx={{ bgcolor: 'white', padding: '10px' }}
                    fullWidth={true}
                    id="search"
                    name="search"
                    label="search"
                    placeholder='EX : Anxiety, Depression, etc'
                    value={values.search}
